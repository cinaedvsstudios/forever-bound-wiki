...
const els = {
  ...
  designExpandAllButton: document.querySelector("#designExpandAllButton"),
  designCollapseAllButton: document.querySelector("#designCollapseAllButton"),
  openTcardDesignerButton: document.querySelector("#openTcardDesignerButton"),
...
function bindEditorEvents() {
  ...
  els.designExpandAllButton?.addEventListener("click", () => setDesignCardsExpanded(true));
  els.designCollapseAllButton?.addEventListener("click", () => setDesignCardsExpanded(false));
  els.openTcardDesignerButton?.addEventListener("click", () => { saveSelectionRange(); toggleBlockPanel(true); });
...
function openCapsDialog(title, fields) {
  els.dialogBox.classList.remove("delete-dialog");
  const footer = els.dialogBox.querySelector("footer");
  if (footer) footer.innerHTML = '<button id="dialogConfirmButton" type="submit">Apply</button>';
...
function openDeleteConfirmDialog({ title, tree = [] }) {
  els.dialogBox.classList.add("delete-dialog");
  els.dialogTitle.textContent = title;
  const hasTree = Array.isArray(tree) && tree.length > 0;
  els.dialogFields.innerHTML = hasTree
    ? `<p class="delete-dialog-message">This section is not empty. You are about to delete:</p><div class="delete-tree-box">${renderDeleteTree(tree)}</div>`
    : `<p class="delete-dialog-message">This item is empty. Confirm before deleting it.</p>`;
  const footer = els.dialogBox.querySelector("footer");
  if (footer) {
    footer.innerHTML = `<label class="dialog-check delete-confirm-check"><input type="checkbox" name="confirm"> I still want to delete this.</label><button id="dialogConfirmButton" type="submit">Confirm</button>`;
  }
  els.dialogOverlay.hidden = false;
  footer?.querySelector("input[name='confirm']")?.focus();
  return new Promise((resolve) => { state.dialogResolver = (data) => resolve(data?.confirm === "on"); });
}

function renderDeleteTree(items) {
  return `<ul>${items.map((item) => `<li><span>${escapeHtml(item.label)}</span>${item.children?.length ? renderDeleteTree(item.children) : ""}</li>`).join("")}</ul>`;
}
...
function handleSettingsCardToggle(event) {
  const heading = event.target.closest(".settings-section > h3, .design-heading-row > h3");
  if (!heading) return;
...
async function handleFilingTabAction(action, tabId) {
  ...
  if (action === "delete") {
    const confirmed = await openDeleteConfirmDialog({
      ...
      tree: tabDocs.length ? [{ label: tab.label, children: tabDocs.map(documentDeleteTreeItem) }] : [],
    });
    if (!confirmed) return;
    moveDocumentsToTrash(tabDocs);
    state.filingTabs = state.filingTabs.filter((item) => item.id !== tabId);
    if (tabDocs.some((doc) => doc.id === state.activeId)) state.activeId = state.documents[0]?.id || createDocument().id;
    renderAll();
    markDirty("Filing Cabinet tab deleted");
  }
}
...
async function handleFilingGroupAction(action, groupId) {
  ...
  if (action === "delete") {
    ...
    if (groupTabs.length || groupDocs.length) {
      const tabItems = groupTabs.map((tabItem) => ({
        label: tabItem.label,
        children: state.documents.filter((doc) => doc.filingTabId === tabItem.id).map(documentDeleteTreeItem),
      }));
      const directDocs = state.documents.filter((doc) => doc.filingGroupId === groupId && !groupTabs.some((tabItem) => tabItem.id === doc.filingTabId)).map(documentDeleteTreeItem);
      tree.push({ label: group.label, children: [...tabItems, ...directDocs] });
    }
    const confirmed = await openDeleteConfirmDialog({ title: "Confirm Delete Folder", tree });
    if (!confirmed) return;
    ...
    if (groupDocs.some((doc) => doc.id === state.activeId)) state.activeId = state.documents[0]?.id || createDocument().id;
    renderAll();
    markDirty("Filing Cabinet folder deleted");
  }
}
...
async function deleteDocumentSafely(docId) {
  ...
  const tree = stats.characters ? [documentDeleteTreeItem(doc)] : [];
  const confirmed = await openDeleteConfirmDialog({ title: "Confirm Delete Document", tree });
  if (!confirmed) return;
...
function documentDeleteTreeItem(doc) {
  const children = [];
  const versions = state.deprecated.filter((entry) => entry.deprecatedOf === doc.id || (!entry.deprecatedOf && entry.title === doc.title));
  versions.forEach((entry) => children.push({ label: `Deprecated version: ${entry.title}` }));
  referencedTCardIds(doc.content).forEach((id) => children.push({ label: `Related TCard: ${id}` }));
  return { label: doc.title || doc.id, children };
}

function referencedTCardIds(html) {
  return [...new Set([...String(html || "").matchAll(/\{\{([A-Za-z]+-[A-Za-z0-9-]+)\}\}/g)].map((match) => match[1]))];
}
...
function bindDesignColorTools() {
  ...
  colorInputs.forEach((input) => {
    ...
    input.addEventListener("click", () => syncActiveColorInput(input));
    input.addEventListener("input", () => {
      syncColorHexPeer(input);
  ...
  els.currentColorBox?.addEventListener("dragstart", (event) => event.dataTransfer?.setData("text/plain", currentActiveColor()));
...
function prepareDesignColorHexControls() {
  document.querySelectorAll(".design-section input[type='color']").forEach((input) => {
    if (input.dataset.hexReady === "true") return;
    input.dataset.hexReady = "true";
    const hex = document.createElement("input");
    hex.type = "text";
    hex.className = "color-hex-input";
    hex.maxLength = 7;
    hex.value = input.value.toUpperCase();
    hex.setAttribute("aria-label", `${input.id || "Color"} hex code`);
    input.insertAdjacentElement("afterend", hex);
    input.parentElement?.classList.add("color-control-label");
    hex.addEventListener("focus", () => syncActiveColorInput(input));
    ...
    hex.addEventListener("change", () => {
      ...
      if (!color) {
        hex.value = input.value.toUpperCase();
        setStatus("Use a hex color like #e88f69", "dirty");
        return;
      }
      input.value = color;
      syncActiveColorInput(input);
...
function renderFavoriteColors() {
  ...
  els.favoriteColors.innerHTML = state.favoriteColors.map((color, index) => {
    ...
    const textColor = readableColorForBackground(color);
    return `<button type="button" draggable="true" data-favorite-index="${index}" data-favorite-color="${escapeAttr(color)}" aria-label="Use ${escapeAttr(text)}" style="--favorite-color:${escapeAttr(color)};--favorite-text:${textColor}"><span>${escapeHtml(text)}</span></button>`;
  }).join("");
}
...
function handleFavoriteColorDrop(event) {
  ...
  if (!button) return;
  event.preventDefault();
  const color = normalizeHexColor(event.dataTransfer?.getData("text/plain") || currentActiveColor());
  if (!color) return;
...
function syncColorHexPeer(input) {
  const peer = input?.parentElement?.querySelector?.(".color-hex-input");
  if (peer) peer.value = input.value.toUpperCase();
}

function currentActiveColor() {
  return normalizeHexColor(els.currentColorBox?.dataset.color || els.activeColorHex?.value || state.lastColorInput?.value || "") || "#000000";
}

function updateCurrentColorBox(value) {
  const color = normalizeHexColor(value);
  if (!color) return;
  if (els.activeColorHex) els.activeColorHex.value = color.toUpperCase();
  if (els.currentColorBox) {
    els.currentColorBox.dataset.color = color;
    els.currentColorBox.style.setProperty("--current-color", color);
    els.currentColorBox.style.setProperty("--current-text", readableColorForBackground(color));
    const strong = els.currentColorBox.querySelector("strong");
    if (strong) strong.textContent = color.toUpperCase();
  }
}
...
function readableColorForBackground(value) {
  const color = normalizeHexColor(value);
  if (!color) return "#fbf4d6";
  const r = parseInt(color.slice(1, 3), 16);
  const g = parseInt(color.slice(3, 5), 16);
  const b = parseInt(color.slice(5, 7), 16);
  return ((r * 299 + g * 587 + b * 114) / 1000) > 150 ? "#211812" : "#fbf4d6";
}

function setDesignCardsExpanded(expanded) {
  document.querySelectorAll(".design-section .design-card").forEach((card) => { card.open = expanded; });
}
...
function loadDesignForm() {
  ...
  document.querySelectorAll(".design-section input[type='color']").forEach(syncColorHexPeer);
